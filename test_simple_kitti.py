...
def test_simple(args):
    ...
    with torch.no_grad():
        for line in lines:
            ...
            input_image = pil.open(example).convert('RGB')
            ...
            if args.pred_metric_depth:
                name_dest_npy = os.path.join(output_folder, date, seqname, subfolder, "{}_depth.npy".format(output_name))
                metric_depth = STEREO_SCALE_FACTOR * depth.cpu().numpy()
                np.save(name_dest_npy, metric_depth)
            else:
                name_dest_npy = os.path.join(output_folder, date, seqname, subfolder, "{}_disp.npy".format(output_name))
            ...
            name_dest_im = os.path.join(output_folder, date, seqname, subfolder, "{}_disp.jpeg".format(output_name))
